describe('Settings', () => {
  ...
  it('enable switch to hymn scene on search', () => {
    ...
    cy.window().then(win => {
      const onSearchSwitchToHymnScene = win.localStorage.getItem('onSearchSwitchToHymnScene')
  ...
  it.skip('set switch to scene on music end', () => {
    ...
    cy.window().then(win => {
      const onMusicEndSwitchToScene = win.localStorage.getItem('onMusicEndSwitchToScene')
